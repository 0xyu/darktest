...
##
## Verifies the router turns a GLOBAL stage number into the right gameplay
...
## BOSS). Phase 7.6: the area is DERIVED from the stage number, so a caller
## cannot pass the wrong one. Stage numbers no area covers resolve to clean
## failed routes.
...
func _test_forest_walkthrough() -> void:
	...
	for number in range(1, 11):
		var route_result: Dictionary = router.route(number)
...
func _test_boss_routes_into_combat_but_keeps_type() -> void:
	var router := StageRouterScript.new()
	var route_result: Dictionary = router.route(10)
...
## The caller only supplies a stage number; the area comes from the authored
## range, so there is no way to ask for the wrong area and no second place that
## could disagree about which area a number belongs to.
func _test_route_derives_the_area() -> void:
	var router := StageRouterScript.new()
	_expect(String(router.route(6).get("area_id", "")) == "forest", "route(6) derives the forest area")
	_expect(String(router.route(10).get("area_id", "")) == "forest", "route(10) derives the forest area")
	_expect(String(StageDatabaseScript.area_for_stage(1)) == "forest", "StageDatabase derives the area for stage 1")
	_expect(String(StageDatabaseScript.area_for_stage(11)) == "", "no area covers stage 11 yet, so none is derived")
...
func _test_destination_for_stage() -> void:
	var router := StageRouterScript.new()
	var authored_combat_stage := StageDatabaseScript.lookup(6)
	...
	var boss_stage := StageDatabaseScript.lookup(10)
...
func _test_unknown_requests_fail_cleanly() -> void:
	var router := StageRouterScript.new()
	_expect(not bool(router.route(0).get("ok", false)), "stage 0 should fail to route")
	# With the shipped content Forest is the only area (1-10), so stage 11 and
	# beyond are covered by no area: there is nothing to route into.
	_expect(not bool(router.route(11).get("ok", false)), "stage 11 should fail to route while no area covers it")
	_expect(not bool(router.route(999).get("ok", false)), "stage 999 should fail to route while no area covers it")
	for number in [0, 11, -3]:
		var route_result: Dictionary = router.route(number)
		_expect(route_result.get("destination") == StageRouterScript.Destination.NONE, "failed route for %d should carry NONE" % number)
		_expect(not String(route_result.get("reason", "")).is_empty(), "failed route for %d should explain its reason" % number)
...
func _test_request_enter_emits_only_for_routable_stages() -> void:
	...
	_expect(router.request_enter(6), "request_enter(6) should succeed")
	_expect(router.request_enter(8), "request_enter(8) should succeed")
	_expect(not router.request_enter(0), "request_enter(0) should be rejected")
	_expect(not router.request_enter(11), "request_enter(11) should be rejected while no area covers it")
