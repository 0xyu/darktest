...
func _run_all() -> void:
	var filter := _parse_filter(OS.get_cmdline_user_args())
...
func _print_suite(result: Dictionary) -> void:
	print("[ui_harness] suite %s" % result.suite)
	for test in result.tests:
		if test.skipped:
			print("  [SKIP] %s (%s)" % [test.test, test.skip_reason])
		elif test.passed:
			print("  [PASS] %s (%d assertions)" % [test.test, test.assertions])
		else:
			print("  [FAIL] %s" % test.test)
